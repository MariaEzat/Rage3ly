  ...
  loadAddressForEditing(addressId: string): void {
    this.websiteService.getShippingAddressById(addressId).subscribe({
      next: (res) => {
        ...
        // Update marker position on map if it exists
        if (this.map && this.marker) {
          const newLatLng = new L.LatLng(address.latitude, address.longitude);
          this.marker.setLatLng(newLatLng);
          this.map.setView(newLatLng, this.map.getZoom()); // Adjust view to new position
        }
      },
      error: (err) => {
    ...
    if (this.route.snapshot.queryParams['id']) {
      ...
      this.websiteService.EditShippingAddress(editedShippingAddress).subscribe({
        next: (res) => {
          ...
          this._router.navigate(['/tahwesha/address']);
        },
        error: (err) => {
          this._sharedService.showToastr(err);
        },
