...
import { CreateOrderByClientViewModel, CreateShippingAddressViewModel } from '../../models/order.model';
  ...
  Save() {
    ...
    this.websiteService.CreateOrder(this.item).subscribe({
      next: (res) => {
        ...
        this.websiteService.productsInCart = [];
        this.websiteService.totalPrice = 0;
        this._sharedService.showToastr(res);
        if (res.isSuccess) {
          this._router.navigate(['/tahwesha/home']);
        }
      },
      error: (err) => {
        this._sharedService.showToastr(err);
