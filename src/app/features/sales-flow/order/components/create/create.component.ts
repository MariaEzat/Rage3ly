...
export class CreateComponent {
  ...
  checkIfEditMode(): boolean {
    // Your logic to determine if it's edit mode
    ...
    if (!this.page.isEdit) {
      ...
      this._orderService.Update(this.editItem).subscribe({
        next: (res) => {
          ...
          this._sharedService.showToastr(res);
          if (res.isSuccess) {
            this._router.navigate(['/salesflow/order/details',this.orderNumber]);
          }
        },
        error: (err) => {
          this._sharedService.showToastr(err);
  ...
  async initMap(): Promise<void> {
    try {
      ...
      // Retrieve stored latitude and longitude from the form
      const storedLat = this.page.form.get('latitude')?.value || 26.8206;
      const storedLng = this.page.form.get('longitude')?.value || 30.8025;
  
      if (!this.map) {
        this.map = L.map(mapContainer).setView([storedLat, storedLng], 6);
        ...
        this.marker = L.marker([storedLat, storedLng], { icon: customIcon, draggable: true })
          .addTo(this.map)
          .bindPopup('Selected location')
          .openPopup();
  
        // Event to update form when clicking on the map
        ...
        // Event to update form when dragging the marker
        this.marker.on('dragend', (event) => {
          const newLatLng = event.target.getLatLng();
          this.updateFormCoordinates(newLatLng.lat, newLatLng.lng);
        });
      }
    } catch (error) {
  ...
  // Function to update form with new coordinates
