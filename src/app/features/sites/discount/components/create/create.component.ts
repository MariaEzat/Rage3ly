...
export class CreateComponent implements OnInit {
  ...
  createForm(): void {
    ...
    this.page.form.get('discountType')?.valueChanges.subscribe((selectedType) => {
      this.toggleAmountField(selectedType);
    });

    this.page.isPageLoaded = true;
  }

  toggleAmountField(type: number): void {
    const amountControl = this.page.form.get('amount');

    if (type === 1) { // "Percentage" selected
      amountControl?.enable();

      // Append '%' for UI display but store only numeric value
      const amount = amountControl?.value ? parseFloat(amountControl.value) : null;
      amountControl?.setValue(amount !== null ? amount + '%' : '');

    } else if (type === 2) { // "Money" selected
      amountControl?.enable();

      // Remove '%' if switching to Money type
      const amount = amountControl.value?.toString().replace('%', '');
  ...
  Save(): void {
    ...
    if (this.item.discountType === 1 && this.item.amount) {
      this.item.amount = Number(this.item.amount.toString().replace('%', '').trim());
    }
