...
export class DashboardComponent extends CrudIndexBaseUtils {
  ...
  constructor(private cd: ChangeDetectorRef, private _dashboardService: DashboardService, public override _sharedService: SharedService, private _orderService: OrderService,private cdr: ChangeDetectorRef,
  ...
  override createSearchForm() {
    this.page.searchForm = this._sharedService.formBuilder.group({
      ...
      orderStatus: [this.searchViewModel.orderStatus]  // Add orderStatus for filtering
    });
  ...
  //           backgroundColor: ['#eb762c', '#717BBC', '#12b76a', '#f04438', '#a6f4c5','#717680']
  //         }
  //       ],
      ...
      this.options = {
        onClick: (event: MouseEvent, elements: any[]) => {
          if (elements.length > 0) {
            ...
            const selectedStatusId = this.orderStatuslist[index]?.orderStatus; // Get the corresponding status ID
            if (selectedStatusId) {
              this.filterOrdersByStatus(selectedStatusId);
            }
          }
        }
      };
  ...
  filterOrdersByStatus(statusId: number) {
    this.searchViewModel.orderStatus = statusId; // Ensure it's a string
    this.page.searchForm.patchValue({ orderStatus: statusId.toString() });
    this.search();
  }
  ...
  getCountClass(statusId: number): string {
    switch (statusId) {
      ...
      case 1: return 'color-orange bg-brand-100';    // Pending
      ...
      case 5: return 'color-green-300 bg-green-100';    // deleverd
      case 6: return 'color-gray-500 bg-gray-200';    // Completed
      default: return '';
    }
  }
